...
<!-- Added: 2026-06-28 -->
- `network_mode: service:X` joins the dependent container to X's network namespace at **creation time**. If X is later recreated (new PID, new namespace inode), the dependent container is left holding a stale namespace — `docker inspect` still shows the correct container ID but the actual namespace no longer matches. Symptom: ports the dependent service listens on don't appear in `ss -tlnp` from inside X, and connections are refused. Diagnose by comparing namespace inodes: `ls -la /proc/$(docker inspect -f '{{.State.Pid}}' <A>)/ns/net` vs the same for X — inodes must match. Fix: `docker compose up -d --force-recreate immichframe`. Rule: whenever the Tailscale container is recreated, all containers with `network_mode: service:tailscale` must be recreated too.
